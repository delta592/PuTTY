/*
 * storage.c: macOS-specific implementation of the interface defined
 * in storage.h.
 *
 * Canonical layout (see paths.h):
 *   ~/Library/Application Support/PuTTY/
 *     sessions/          saved sessions
 *     sshhostkeys        host key cache
 *     putty.rnd          random seed
 *     sshhostcas/        CA records
 */
...
static char *make_filename(int index, const char *subname)
{
    ...
    if (index == INDEX_DIR) {
        ...
        return putty_macos_app_support_directory();
    }
    ...
    if (index == INDEX_RANDSEED) {
        ...
        if (env)
            return dupstr(env);
        tmp = make_filename(INDEX_DIR, NULL);
        ret = dupprintf("%s/" PUTTY_MACOS_RANDOMSEED_BASENAME, tmp);
        sfree(tmp);
        return ret;
    }
...
settings_w *open_settings_w(const char *sessionname, char **errmsg)
{
    ...
    *errmsg = NULL;
    ...
    /*
     * Start by making sure the Application Support directory and its
     * sessions subdir actually exist.
     */
...
static int remove_path(const char *path, const struct stat *sb,
...
void cleanup_all(void)
{
    ...
    if (access(dir, F_OK) == 0) {
        nftw(dir, remove_path, 64, FTW_DEPTH | FTW_PHYS);
        rmdir(dir);
    }
