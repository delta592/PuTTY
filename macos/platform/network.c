...
struct NetSocket {
    ...
    SockAddrStep step;
    /*
     * macOS 15 Local Network privacy (TN3179): BSD connect fails with
     * EHOSTUNREACH while the Allow alert is pending. Retry briefly so
     * the first LAN SSH after an ad-hoc rebuild can succeed once the
     * user grants access (Network.framework would wait; we cannot).
     *
     * WORKAROUND: TN3179 BSD-socket guidance — see .cursor/rules/agents.mdc
     */
    int lnp_generation;
    int lnp_retries_left;
    bool lnp_retry_pending;
    /*
...
static SockAddr sk_extractaddr_tmp(SockAddr *addr, const SockAddrStep *step);
static void mac_lnp_schedule_retry(NetSocket *s);
static bool mac_lnp_is_privacy_errno(int err);

/*
 * How long to keep retrying EHOSTUNREACH / ENETUNREACH while macOS may
 * be showing the Local Network privacy alert (TN3179).
 */
#define MAC_LNP_RETRY_MS        500
#define MAC_LNP_MAX_RETRIES     40   /* 20 seconds */

struct mac_lnp_retry_ctx {
    NetSocket *sock;
    int generation;
};
...
static bool mac_netsocket_in_tree(NetSocket *s)
{
    int i;
    NetSocket *t;

    for (i = 0; (t = index234(sktree, i)) != NULL; i++) {
        if (t == s)
            return true;
    }
    return false;
...
static void mac_lnp_retry_fire(void *vctx)
{
    struct mac_lnp_retry_ctx *ctx = (struct mac_lnp_retry_ctx *)vctx;
    NetSocket *s = ctx->sock;
    ...
    /*
     * sk_net_close bumps lnp_generation and removes the socket from
     * sktree before sfree. Either check means this retry is stale.
     */
    if (!mac_netsocket_in_tree(s) || s->lnp_generation != generation)
        return;

    s->lnp_retry_pending = false;

    if (!s->addr || s->connected)
        return;

    START_STEP(s->addr, s->step);
    do {
        err = try_connect(s);
    } while (err && !mac_lnp_is_privacy_errno(err) &&
             sk_nextaddr(s->addr, &s->step));

    if (!err) {
        /* Connected immediately, or EINPROGRESS with uxsel armed. */
        return;
    }

    if (mac_lnp_is_privacy_errno(err) && s->lnp_retries_left > 0) {
        mac_lnp_schedule_retry(s);
        return;
    }

    /* Permanent failure (or retries exhausted). */
    {
        char *errmsg = dupstr(strerror(err));
...
static void mac_lnp_schedule_retry(NetSocket *s)
{
    struct mac_lnp_retry_ctx *ctx;

    if (!s || s->lnp_retry_pending || s->lnp_retries_left <= 0)
        return;

    s->lnp_retry_pending = true;
    s->lnp_retries_left--;

    ctx = snew(struct mac_lnp_retry_ctx);
    ctx->sock = s;
    ctx->generation = s->lnp_generation;

    dispatch_after_f(
        dispatch_time(DISPATCH_TIME_NOW,
                      (int64_t)MAC_LNP_RETRY_MS * NSEC_PER_MSEC),
               ...
               bool nodelay, bool keepalive, Plug *plug)
{
    ...
    s->port = port;
    s->lnp_generation = 0;
    s->lnp_retries_left = MAC_LNP_MAX_RETRIES;
    s->lnp_retry_pending = false;
    ...
    if (mac_lnp_is_privacy_errno(err)) {
        /*
         * Do not surface EHOSTUNREACH immediately — Local Network
         * privacy often denies the first BSD connect while the Allow
         * alert is still up (TN3179). Keep the socket error-free and
         * retry on the main queue.
         */
        START_STEP(s->addr, s->step);
        mac_lnp_schedule_retry(s);
    } else if (err) {
...
static void sk_net_close(Socket *sock)
{
    ...
    /* Invalidate any pending Local Network privacy connect retries. */
    s->lnp_generation++;
    s->lnp_retry_pending = false;
        ...
        if (!s->connected) {
             ...
             */
            {
                ...
                if (errmsg) {
                    ...
                    if (err) {
                        /*
                         * Local Network privacy can report EHOSTUNREACH
                         * on the writable completion path as well as on
                         * the initial connect(). Retry instead of
                         * closing while the Allow alert may still be up.
                         */
                        if (mac_lnp_is_privacy_errno(err) &&
                            s->lnp_retries_left > 0) {
                            START_STEP(s->addr, s->step);
